...
func GenerateWire(input string) Wire {
	var wires Wire
	...
	for _, move := range moves {
		...
		switch {
		case direction == "R":
			endPoint = Point{currentPoint.x + distance, currentPoint.y}
		case direction == "U":
			endPoint = Point{currentPoint.x, currentPoint.y + distance}
		case direction == "L":
			endPoint = Point{currentPoint.x - distance, currentPoint.y}
		case direction == "D":
			endPoint = Point{currentPoint.x, currentPoint.y - distance}
		}
		currentSegment := Segment{currentPoint, endPoint}
		wires.segments = append(wires.segments, currentSegment)
		currentPoint = endPoint
	}
	return wires
}

// This is the ugliest thing I have done in my life. I feel shame, and you should too for reading. But, it solved my problem
func Intersection(wire1 Wire, wire2 Wire) []Point {
	visitedMap := make(map[Point]bool)
	var intersections []Point
	fmt.Println("GenerateWire 1")
	for _, segment := range wire1.segments {
		if segment.start.x != segment.end.x {
			for i := segment.start.x; ; {
				if i == segment.end.x {
					break
				}
				if segment.start.x > segment.end.x {
				...
				num := i
				fmt.Printf("x: %d, y: %d | ", num, segment.start.y)
				visitedMap[Point{num, segment.start.y}] = true
			}
		} else {
			for i := segment.start.y; ; {
				if i == segment.end.y {
					break
				}
				...
				num := i
				fmt.Printf("x: %d, y: %d | ", segment.start.x, num)
				visitedMap[Point{segment.start.x, num}] = true
			}
		}
	}
	delete(visitedMap, Point{0, 0})
	fmt.Println("\nGenerateWire 2")
	for _, segment := range wire2.segments {
		if segment.start.x != segment.end.x {
			for i := segment.start.x; ; {
				...
				num := i
				fmt.Printf("x: %d, y: %d | ", num, segment.start.y)
				if (visitedMap[Point{num, segment.start.y}] == true) {
					intersections = append(intersections, Point{num, segment.start.y})
				}
			}
		} else {
			for i := segment.start.y; ; {
				...
				num := i
				fmt.Printf("x: %d, y: %d | ", segment.start.x, num)
				if (visitedMap[Point{segment.start.x, num}] == true) {
					intersections = append(intersections, Point{segment.start.x, num})
				}
			}
		}
	}
